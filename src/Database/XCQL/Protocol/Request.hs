...
encodeStartup (Startup v c) = encodeMap $ ("CQL_VERSION", mapVersion v) : mapCompression c
  where
...
encodeQuery v (Query (QueryString s) p) = encodeLongString s >> encodeQueryParams v p
...
encodeExecute v (Execute (QueryId q) p) = encodeShortBytes q >> encodeQueryParams v p
...
encodeQueryParams v p = do
  ...
  where
    ...
    queryFlags =
        ...
        (if isJust (pageSize p)
             then 0x04
             else 0x0) .|.
        (if isJust (queryPagingState p)
             then 0x08
             else 0x0) .|.
        (if isJust (serialConsistency p)
             then 0x10
             else 0x0)
