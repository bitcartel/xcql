...
import qualified Database.XCQL.Protocol.Tuple.TH as Tuples
...
instance Cql UTCTime where
    ...
    toCql = CqlTimestamp . truncate . (* 1000) . utcTimeToPOSIXSeconds
...
instance (Cql a, Cql b) => Cql (Map a b) where
    ctype = Tagged $ MapColumn (untag (ctype :: Tagged a ColumnType)) (untag (ctype :: Tagged b ColumnType))
